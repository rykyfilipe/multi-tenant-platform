...
describe('TableWidgetProcessor', () => {
  ...
  describe('process', () => {
    it('should process data correctly with summary row', () => {
      ...
      // With independent aggregations: AVG([1000, 1500, 800]) = 1100 (last aggregation)
      expect(result.summary?.sales?.avg).toBe(1100); // Independent result
    ...
    it('should process grouped aggregation correctly', () => {
      ...
      // With independent aggregations: AVG([1000, 800]) = 900 (last aggregation)
      expect(result.data[0].sales).toBe(900); // Final result stored with column name
      expect(result.data[0].sales_avg).toBe(900); // Also stored with last aggregation function
      expect(result.data[1]._groupKey).toBe('South');
      expect(result.data[1]._count).toBe(1);
      expect(result.data[1].sales).toBe(1500); // AVG([1500]) = 1500
    });
