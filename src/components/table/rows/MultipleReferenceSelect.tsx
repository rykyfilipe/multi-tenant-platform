...
export function MultipleReferenceSelect({
	...
	const selectedOptions = useMemo(() => {
		...
		const filtered = transformedOptions.filter((opt) => {
			...
			if (selectedValues.length > 0) {
				console.log("MultipleReferenceSelect: Checking option:", {
					optPrimaryKey,
					optDisplayValue: opt.displayValue,
					selectedValues,
					isSelected,
					comparison: selectedValues.map((val) => ({
						val: val?.toString(),
						matches: val?.toString() === optPrimaryKey,
					})),
				});
			}
		...
		console.log("MultipleReferenceSelect: selectedOptions result:", {
			transformedOptionsCount: transformedOptions.length,
	...
	// Debug logging to help identify data inconsistencies
	useEffect(() => {
		if (isMultiple && selectedValues.length > 0) {
			console.log("MultipleReferenceSelect Debug:", {
				selectedValues,
				...
				transformedOptions: transformedOptions.slice(0, 3), // Show first 3 options
			});
		}
	}, [
		selectedValues,
		...
		if (
			...
			if (missingValues.length > 0) {
				...
				if (validValues.length !== value.length) {
					console.log(
						"MultipleReferenceSelect: Auto-cleaning invalid values:",
						{
							original: value,
							cleaned: validValues,
						},
	...
	const toggleOption = (option: ReferenceOption) => {
		console.log("MultipleReferenceSelect: toggleOption called with:", {
			option,
			optionPrimaryKeyValue: option.primaryKeyValue,
			optionId: option.id,
		...
		if (!optionValue) {
			console.error(
				"MultipleReferenceSelect: No primary key value found for option:",
				option,
			);
			// Don't return early - just log the error and continue
			console.warn("MultipleReferenceSelect: Skipping invalid option");
			return;
		}

		if (isMultiple) {
			console.log("MultipleReferenceSelect: Toggling option:", {
				optionValue,
				optionDisplayValue: option.displayValue,
				primaryKeyValue: option.primaryKeyValue,
				rowId: option.id,
				currentSelectedValues: selectedValues,
				willAdd: !selectedValues.some((val) => val?.toString() === optionValue),
			});
			...
			console.log("MultipleReferenceSelect: Selection state:", {
				isCurrentlySelected,
				optionValue,
				selectedValues,
			});
			...
			console.log("MultipleReferenceSelect: New values after toggle:", {
			...
			console.log("MultipleReferenceSelect: onValueChange called successfully");
		} else {
			// Single selection - replace current value
			console.log(
				"MultipleReferenceSelect: Single selection, setting value to:",
				optionValue,
			);
	...
	const validateSelectedValues = () => {
		...
		console.log("MultipleReferenceSelect: validateSelectedValues:", {
			selectedValues,
			invalidValues,
	...
	const getValidSelectedValues = () => {
		...
		console.log("MultipleReferenceSelect: getValidSelectedValues:", {
			selectedValues,
			validValues,
			transformedOptionsCount: transformedOptions.length,
		});
	...
	const clearSelection = () => {
		console.log("MultipleReferenceSelect: Clearing selection:", {
			currentSelectedValues: selectedValues,
	...
	const removeOption = (optionValue: string) => {
		if (isMultiple) {
			console.log("MultipleReferenceSelect: Removing option:", {
				optionValue,
				currentSelectedValues: selectedValues,
				selectedOptionsCount: selectedOptions.length,
			});
			...
			console.log(
				"MultipleReferenceSelect: New values after removal:",
				newValues,
			);
