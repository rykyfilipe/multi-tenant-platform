...
export default function StandaloneApiConsumer() {
	// Configuration state
	...
	const testConnection = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				toast.error(
					`API Error: ${error.error || "Invalid token or configuration"}`,
				);
			}
		} catch (err) {
			toast.error("Failed to connect to API. Please check your configuration.");
			console.error(err);
		} finally {
			setLoading(false);
		}
	...
	const fetchTableDetails = useCallback(
		async (tableId: number) => {
			try {
				...
				const result: ApiResponse<TableData & { columns: ColumnData[] }> =
					await response.json();
	...
	const fetchRows = useCallback(
		async (tableId: number, page: number = 1, limit: number = 50) => {
			try {
				setLoading(true);
				setError(null);
				...
				const response = await fetch(
					`${baseUrl || ""}/api/public/tables/${tableId}/rows?${params}`,
					{ headers },
				);
			...
			<div className='max-w-2xl mx-auto p-6'>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Enter your API token and base URL to start consuming public table
							data
						</CardDescription>
					</CardHeader>
				...
				<Alert className='mt-6'>
					<AlertDescription>
						<strong>Note:</strong> This component requires a valid JWT token
						with access to public tables. Make sure your token has the necessary
					...
					<CardHeader>
						<div className='flex items-center justify-between'>
							<div>
								...
								<CardDescription>
									{selectedTable.description || "No description"} • {totalRows}{" "}
									total rows
								</CardDescription>
							</div>
