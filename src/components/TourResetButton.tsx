	...
	return (
		<div className={`relative ${className}`}>
			<Button
				...
				title={t("tour.resetButton.title")}>
				<RefreshCw className='w-4 h-4' />
				{showConfirmation && showConfirm ? t("tour.resetButton.confirmReset") : t("tour.resetButton.resetTours")}
			</Button>
			...
			{showConfirmation && showConfirm && (
				<div className='absolute top-full left-0 mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-md text-xs text-yellow-800 max-w-xs'>
					{t("tour.resetButton.confirmationMessage")}
				</div>
			)}
		</div>
	);
