...
const SubscriptionManager: React.FC<SubscriptionManagerProps> = ({
		...
		try {
			...
			if (!response.ok) {
				if (data.error && data.error.includes("not configured")) {
					showAlert(
						"Customer portal is not configured. Please contact support.",
						"error",
					);
					return;
				}
	...
	const handleCancelSubscription = async () => {
		...
		try {
			...
			showAlert("Failed to cancel subscription. Please try again.", "error");
		} finally {
			setIsActionLoading(false);
		}
	};
					...
					<CardContent>
						<div className='text-center py-8'>
							...
							<p className='text-muted-foreground mb-6 max-w-md mx-auto'>
								Get started with our Starter plan and unlock powerful database
								management features.
							</p>
									...
									<div className='flex-1 flex items-center justify-center p-3 bg-muted/50 rounded-md border border-dashed'>
										<div className='text-center'>
											...
											<p className='text-xs text-muted-foreground'>
												Starter plan cannot be canceled or downgraded
											</p>
										</div>
									</div>
								)}
			...
			<Dialog open={showDowngradeDialog} onOpenChange={setShowDowngradeDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Downgrade to a lower plan. Changes will take effect at the end of
					...
					<DialogFooter>
						...
						<Button onClick={handleDowngradePlan} variant='outline'>
							<ArrowDownRight className='w-4 h-4 mr-2' />
							Downgrade Now
						</Button>
					</DialogFooter>
